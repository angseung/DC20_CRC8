...
unsigned char cal_crc(unsigned char *input_data, int dat_len)
{
...
void flip_message(void){
    ...
    if (order == 99){
        ...
        for (i = start_val; i < start_val + flip_num; i++){
            j = i / 8;
            k = i % 8;
...
void flip_message_auto(int s_idx, int nums){
    int order;
    int start_val, flip_num;
    int i, j, k;
...
int main(void){
    ...
    unsigned char crc8_val;
    unsigned char crc_check;
    int i;
    int end_idx = sizeof(input_val) / sizeof(unsigned char) - 1;
    
    crc8_val = cal_crc(input_val, 3);
    ...
    for (int i = 0; i<4; i++){
        if (PRINT_DEBUG_OPT) printf("%d Th Value : %x \n", i, input_val[i]);
    }
    ...
    crc_check = cal_crc(input_val, end_idx);
    if (PRINT_DEBUG_OPT) printf("is correct? %d \n", crc_check);
    
    if (!crc_check) printf("CRC CHECK SUCCESSFUL!, CRC Remainder : 0x%.2x\n", crc_check);
    else printf("CRC CHECK FAIL!, CRC Remainder : 0x%.2x\n", crc_check);
